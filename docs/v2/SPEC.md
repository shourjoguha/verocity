...
> Status: **starting spec for review.** This document is staged in the existing
> Verocity (Lovable) repo. Once approved, it moves to a fresh repository that
> hosts the v2 build. Decisions marked **LOCKED** were confirmed; items under
...
- **Scale target: < 100 profiles.** Small, known user set. This permits an
  invite-gated signup and communal reads without heavy multi-tenant machinery.
